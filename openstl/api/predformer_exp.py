...
class PredFormerExperiment(object):
    ...
    def train(self):
        ...
        for epoch in range(self._epoch, self._max_epochs):
            ...
            if self._rank == 0:
                ...
                mlflow.log_metric('train_loss_epoch', loss_mean.avg, step=epoch)
                mlflow.log_metric('val_loss_epoch', vali_loss, step=epoch)
                mlflow.log_metric('learning_rate', cur_lr, step=epoch)
    ...
    def test(self):
        ...
        mlflow.log_metric('test_mse', eval_res['mse'])
        mlflow.log_metric('test_mae', eval_res['mae'])

        idx = 0
        x = datetime.datetime.now()
        path = Path(f"outputs_{self.experiment_name}_{x.strftime('%Y_%m_%d')}")
        path.mkdir(parents=True, exist_ok=True)

        for inputs, pred, true in zip(results["inputs"][0:100], results["preds"][0:100], results["trues"][0:100]):
            previous_image = inputs[-1][0]
            pred_image = pred.squeeze(0)[0]
            true_image = true.squeeze(0)[0]

            self.log_overlay(path, previous_image, pred_image, true_image, idx)
            self.log_growth(path, inputs, idx)
            idx += 1
    ...
    def create_overlay(self, image_one, image_two, title):
        image_one = (image_one * 255) > 127
        image_two = (image_two * 255) > 127

        one_mask = (image_one).astype(int)
        two_mask = 2 * (image_two).astype(int)

        diff_mask = one_mask + two_mask

        overlay = np.zeros((*one_mask.shape[0:2], 3))
        overlay[:,:,0] = diff_mask
        overlay[:,:,1] = diff_mask
        overlay[:,:,2] = diff_mask

        # Areas where both overlap will equal to 3
        overlay[np.all(overlay == [3, 3, 3], axis=-1)] = [255, 255, 255]
        overlay[np.all(overlay == [1, 1, 1], axis=-1)] = [0, 255, 0]
        overlay[np.all(overlay == [2, 2, 2], axis=-1)] = [0, 0, 255]

        overlay = cv2.putText(overlay, title, (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)

        return overlay

    def log_overlay(self, path, previous, pred, true, idx):
        pred_true_overlay = self.create_overlay(pred, true, "Pred/True")
        previous_true_overlay = self.create_overlay(true, previous, "True/Previous")
        pred_previous_overlay = self.create_overlay(pred, previous, "Pred/Previous")

        overlay = np.concatenate((pred_true_overlay, pred_previous_overlay, previous_true_overlay), axis=1)
        
        cv2.imwrite(str(path / f"overlay_debug_{idx}.png"), overlay)

    def log_growth(self, path, inputs, idx):

        whitened = (inputs * 255) > 127

        whitened = whitened.squeeze(1).astype(int)

        growth_image = np.concatenate(whitened, axis=1)

        growth_image_rgb = np.zeros((*growth_image.shape[0:2], 3))
        growth_image_rgb[:,:,0] = growth_image
        growth_image_rgb[:,:,1] = growth_image
        growth_image_rgb[:,:,2] = growth_image

        growth_image_rgb[np.all(growth_image_rgb == [1, 1, 1], axis=-1)] = [255, 255, 255]

        cv2.imwrite(str(path / f"growth_{idx}.png"), growth_image_rgb)