...
from modules import Eidetic3DLSTMCell, tf_Conv3d
...
class E3DLSTM_Model(nn.Module):
    def __init__(self, num_layers, num_hidden, configs, **kwargs):
        ...
        for i in range(num_layers):
            ...
            cell_list.append(
                Eidetic3DLSTMCell(in_channel, num_hidden[i], self.window_length, height, width, (2, 5, 5),
                                       configs.stride, configs.layer_norm)
            )
        self.cell_list = nn.ModuleList(cell_list)
        self.conv_last = nn.Conv3d(num_hidden[num_layers - 1], self.frame_channel,
                                   kernel_size=(self.window_length, 1, 1), stride=(self.window_length, 1, 1), padding=0, bias=False)
