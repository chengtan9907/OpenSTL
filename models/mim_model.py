...
class MIM_Model(nn.Module):
    def __init__(self, num_layers, num_hidden, configs, **kwargs):
        ...
        for i in range(num_layers):
            ...
            if i < 1:
                stlstm_layer.append(SpatioTemporalLSTMCell(in_channel, num_hidden[i], height, width, configs.filter_size,
                                        configs.stride, configs.layer_norm))
            else:
                stlstm_layer.append(MIMBlock(in_channel, num_hidden[i], height, width, configs.filter_size,
                                        configs.stride, configs.layer_norm))
        
        for i in range(num_layers-1):
            stlstm_layer_diff.append(MIMN(num_hidden[i], num_hidden[i+1], height, width, configs.filter_size,
                                    configs.stride, configs.layer_norm))
    ...
    def forward(self, frames_tensor, mask_true):
        ...
        st_memory = torch.zeros([batch, self.num_hidden[0], height, width]).to(self.configs.device)
