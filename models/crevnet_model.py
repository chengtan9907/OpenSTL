...
from modules import zig_rev_predictor, autoencoder


class CrevNet_Model(nn.Module):
    def __init__(self, in_shape, rnn_size, batch_size, predictor_rnn_layers, pre_seq_length, aft_seq_length, n_eval, **kwargs):
        ...
        self.frame_predictor = zig_rev_predictor(rnn_size, rnn_size, rnn_size, predictor_rnn_layers, batch_size)

        self.encoder = autoencoder(nBlocks=[4,5,3], nStrides=[1, 2, 2],
                            nChannels=None, init_ds=2,
                            dropout_rate=0., affineBN=True, in_shape=[channels, image_height, image_width],
                            mult=2)
